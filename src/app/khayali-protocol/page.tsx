...
export default async function KhayaliProtocolPage() {
  const content = await getContentByCategory('khayali-protocol');

  return (
    <div className="container mx-auto px-4 py-12">
      <h1 className="text-4xl font-bold mb-8 bg-gradient-to-r from-purple-600 to-indigo-600 text-transparent bg-clip-text">
        Khayali Protocol
      </h1>
      ...
      <div className="grid gap-6">
        {content.map((item) => (
          <Link
            href={`/khayali-protocol/${item.slug}`}
            key={item.slug}
